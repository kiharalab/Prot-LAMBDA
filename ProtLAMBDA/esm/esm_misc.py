...
class PairToSequence(nn.Module):
    ...
    def forward(self, pairwise_state):
        ...
        #print('pairwise_state',torch.isnan(pairwise_state).any(),torch.isinf(pairwise_state).any(),)
        z = checkpoint.checkpoint(self.custom(self.layernorm), pairwise_state, use_reentrant=False)
        #print('z',torch.isnan(z).any(),torch.isinf(z).any(),)
        #pairwise_bias = self.linear(z)
        pairwise_bias = checkpoint.checkpoint(self.custom(self.linear), z, use_reentrant=False)
        #print('pairwise_bias',torch.isnan(pairwise_bias).any(),torch.isinf(pairwise_bias).any(),)
...
class ResidueMLP(nn.Module):
    def __init__(self, embed_dim, inner_dim, norm=nn.LayerNorm, dropout=0):
        ...
        self.drpout = nn.Dropout(dropout)
    ...
    def forward(self, x):
        ...
        return x + self.drpout( checkpoint.checkpoint(self.custom(self.mlp), x, use_reentrant=False) )
